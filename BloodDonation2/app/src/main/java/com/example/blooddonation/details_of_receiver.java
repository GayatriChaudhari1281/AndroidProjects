    ...
    public  void ADD_RECIEVER(View view ){
        ...
        if(isvalid == true) {
            if (db.addreciever(new Recievers(name1, Bldgrp, loc,email,contact)) == true) {
                Toast.makeText(details_of_receiver.this, " Receiver Registered", Toast.LENGTH_LONG).show();

            } else {
                Toast.makeText(details_of_receiver.this, "Receiver not Registered", Toast.LENGTH_LONG).show();
            }
        }
    }
